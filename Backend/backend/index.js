...
const {
  ...
  viewCategory,
  searchCategory,
} = require("./src/controllers/cate.controller.js");
...
const corsOptions = {
  origin:[ "http://localhost:3000","http://localhost:3001"], // Allow requests from any origin
...
app.use("/ap/v1/addloc", verifyToken, addlocrouter); // handle by frontend

//3. Route for CRUD, with all users(Buyers, sellers)
app.use("/api/v1/fromDB", verifyToken, fromdbRouter); // admin api more will be added

// to see myself profile, order request, add menu seller
app.use("/api/v1/myprf", verifyToken, profileRouter);

//4.subscription
app.use("/api/v1/cost", verifyToken, subscribeRouter); // drop for now

//5.order , creation will be done
...
app.post("/api/v1/cate/search", verifyToken, (req, res) =>
  searchCategory(req, res, io)
);
